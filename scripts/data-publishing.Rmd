---
title: "KHOJ - Data Tables - For Publishing"
date: "`r Sys.Date()`"
output: 
  html_document:
    df_print: default
...
state_file_dir <- "../data/final-files/state_files/csv/v4/"
all_state_files <- dir(state_file_dir) %>% stringr::str_to_lower()
...
state_df$courtTitle <- stringr::str_replace_all(string = state_df$courtTitle, pattern = "khoj_",replacement = "") %>% stringr::str_replace_all(pattern = "\\.csv",replacement = "") %>% stringr::str_to_title()
...
# This is the path of updated consolidated file (All judges across all high courts)
data_for_analysis_path <- "../data/final-files/master_file/v1.n/KHOJ 1.8.csv"
data_for_analysis <-
...
data_for_analysis$`Schooling Information` <- stringr::str_to_title(data_for_analysis$`Schooling Information`)
data_for_analysis$`Law Degree Institution` <- stringr::str_to_title(data_for_analysis$`Law Degree Institution`)
data_for_analysis$`Litigation Experience` <- stringr::str_to_title(data_for_analysis$`Litigation Experience`)
data_for_analysis$`Chamber Details` <- stringr::str_to_title(data_for_analysis$`Chamber Details`)
data_for_analysis$`Graduation Institution` <- stringr::str_to_title(data_for_analysis$`Graduation Institution`)
data_for_analysis$`Graduation Specialization` <- stringr::str_to_title(data_for_analysis$`Graduation Specialization`)
data_for_analysis$`Law Degree Year` <- stringr::str_to_title(data_for_analysis$`Law Degree Year`)
data_for_analysis$`Bar Enrolment Year` <- stringr::str_to_title(data_for_analysis$`Bar Enrolment Year`)
data_for_analysis$`Post-Graduate in another subject` <- stringr::str_to_title(data_for_analysis$`Post-Graduate in another subject`)
data_for_analysis$`State Bar Association where enrolled` <- stringr::str_to_title(data_for_analysis$`State Bar Association where enrolled`)
```
...
Profile details available for **`r nrow(data_for_analysis)` ** judges across **`r length(unique(data_for_analysis$fileTitle))` ** high courts appointed in the **last 28 years (6th October 1993 - 31st May 2021) ** . 


_Total number of judges in each state level file. Here we are only counting the number of rows in each file. _
...
transfer_1 <- data_for_analysis[data_for_analysis$`If transferred to any other High Court` == "Yes",c("fileTitle","Parent High Court","If yes, which High Court 1")]
names(transfer_1)[] <- c("fileTitle","HC_from","HC_to")

transfer_2 <- data_for_analysis[data_for_analysis$`If transferred to any other High Court` == "Yes",c("fileTitle","If yes, which High Court 1","If yes, which High Court 2")]
names(transfer_2)[] <- c("fileTitle","HC_from","HC_to")
transfer_2 <- transfer_2[!transfer_2$HC_to %in% c("Not Applicable"),]


transfer_3 <- data_for_analysis[data_for_analysis$`If transferred to any other High Court` == "Yes",c("fileTitle","If yes, which High Court 2","If yes, which High Court 3")]
names(transfer_3)[] <- c("fileTitle","HC_from","HC_to")
transfer_3 <- transfer_3[!transfer_3$HC_to %in% c("Not Applicable"),]
...
transferred_any$total_transfers <-
  ifelse(
    !transferred_any$`If yes, which High Court 3` %in% c("Not Applicable"),
    3,
    ifelse(
      !transferred_any$`If yes, which High Court 2` %in% c("Not Applicable"),
      2,
      1
    )
  )
...
### Q8/Q9 - School and Law Degree data availability

```{r school-law-deg-info, echo=FALSE, message=FALSE, warning=FALSE}

school_na <- data_for_analysis %>% filter(`Schooling Information`=="Not Available") %>% group_by(fileTitle) %>% summarise(school_info_na=n())

law_deg_na <- data_for_analysis %>% filter(`Law Degree Institution`=="Not Available") %>% group_by(fileTitle) %>% summarise(law_degree_info_na=n())

total_judges <- data_for_analysis %>% group_by(fileTitle) %>% summarise(totalJudges=n())

law_total <- left_join(total_judges, law_deg_na, by = NULL)

info_na <-
  left_join(law_total, school_na, by = NULL)

info_na[is.na(info_na)] <- 0

info_na <- info_na[,c("fileTitle","school_info_na","law_degree_info_na","totalJudges")]

info_na <- info_na %>% mutate(sna_percent=round(school_info_na/totalJudges*100,2),
                              lna_percent = round(law_degree_info_na/totalJudges*100,2))
...
fdl[is.na(fdl)] <- 0
fdl <- fdl %>% mutate(yes_percent = round(Yes / (Yes + No) * 100, 2))
fdl$yes_percent[is.na(fdl$yes_percent)] <- 0
DT::datatable(
  fdl,
  filter = 'top',
  options = list(autoWidth = TRUE)
...
pgdl[is.na(pgdl)] <- 0
pgdl <- pgdl %>% mutate(yes_percent = round(Yes / (Yes + No) * 100, 2))
pgdl$yes_percent[is.na(pgdl$yes_percent)] <- 0
...
total_cj <- data_for_analysis %>% filter(`If appointed Chief Justice in any High Court` == "Yes") %>% nrow

total_female_cj <- data_for_analysis %>% filter(`If appointed Chief Justice in any High Court` == "Yes" & Gender == "Female") %>% nrow

cj_cadre <- data_for_analysis %>% filter(`If appointed Chief Justice in any High Court` == "Yes") %>% group_by(Cadre) %>% summarise(total=n()) %>% pivot_wider(names_from=Cadre, values_from=total) %>% mutate(bar_percent=round(Bar/(Bar+Service)*100,2), service_percent=round(Service/(Bar+Service)*100,2))
...
cji_data_for_viz <- data.frame("Cadre"=c("Bar","Service"),"Percent Judges"=c(cj_cadre$bar_percent[1], cj_cadre$service_percent[1]))
```
...
```{r chamber-details, echo=FALSE, message=FALSE, warning=FALSE}

chamber_details <- data_for_analysis %>% group_by(`Litigation Experience`, `Chamber Details`) %>% summarise(totalJudges=n()) 
chamber_details <- chamber_details[chamber_details$`Litigation Experience` == "Yes",] %>% mutate(percentJudges = round(totalJudges/sum(totalJudges)*100,2))
...
### Q14 - Data Unavailability

```{r data-unavailability, echo=FALSE, message=FALSE, warning=FALSE}
total_cols <- ncol(data_for_analysis) - 1
total_rows <- nrow(data_for_analysis)

na_df <- c()
for(i in 1:total_cols){
 col_name <- names(data_for_analysis[,i])
 total_na <- nrow(data_for_analysis[data_for_analysis[,i] == "Not Available",])
 total_not_applicable <- nrow(data_for_analysis[data_for_analysis[,i] == "Not Applicable",]) 
 percent_na <- round(total_na/(total_rows-total_not_applicable)*100,2)
 na_i_df <- data.frame("col"=col_name, "total_na"=total_na,"total_not_app" = total_not_applicable,"percent_na"=percent_na)
 na_df <- bind_rows(na_df, na_i_df)
}
...
1. [Visual for Q1](https://www.datawrapper.de/_/3xI7Q/)
2. [Visual for Q2](https://www.datawrapper.de/_/szxYA/)
3. [Visual for Q3](https://www.datawrapper.de/_/m1Hkx/)
4. [Visual for Q4](https://www.datawrapper.de/_/YFEO6/)
5. [Visual for Q6 - 1](https://www.datawrapper.de/_/NMgJl/)
6. [Visual for Q6 - 2](https://www.datawrapper.de/_/qISLd/)
7. [Visual for Q6 - 3](https://www.datawrapper.de/_/oIhKs/)
8. [Visual for Q7](https://www.datawrapper.de/_/IUSYP/)
9. [Visual for Q8/Q9](https://www.datawrapper.de/_/j0Tsz/)
10. [Visual for Q10](https://www.datawrapper.de/_/zpAmL/)
11. [Visual for Q11](https://www.datawrapper.de/_/rvhyT/)
12. [Visual for Q12](https://www.datawrapper.de/_/5Sjhd/)
13. [Visual for Q13](https://www.datawrapper.de/_/1NeXB/)
14. [Visual for Q14](https://www.datawrapper.de/_/QCYbQ/)
