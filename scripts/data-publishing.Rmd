...
fdl <- data_for_analysis %>% group_by(fileTitle, `Foreign Degree in Law`) %>% summarise(total=n()) %>% pivot_wider(names_from = `Foreign Degree in Law`,values_from = total) %>% mutate(yes_percent=round(Yes/(Yes+No)*100,2))
...
pgdl <- data_for_analysis %>% group_by(fileTitle, `Post-Graduate in Law`) %>% summarise(total=n()) %>% pivot_wider(names_from = `Post-Graduate in Law`,values_from = total) %>% mutate(yes_percent=round(Yes/(Yes+No)*100,2))
