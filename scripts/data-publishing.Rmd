...
cj_cadre <- data_for_analysis %>% filter(`If appointed Chief Justice in another High Court` == "Yes") %>% group_by(Cadre) %>% summarise(total=n()) %>% pivot_wider(names_from=Cadre, values_from=total) %>% mutate(bar_percent=round(Bar/(Bar+Service)*100,2), service_percent=round(Service/(Bar+Service)*100,2))
